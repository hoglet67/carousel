...
# $Id: hex2bin.py,v 1.20 2008/11/30 22:17:50 richevans Exp $
...
class Hex2Bin :
    ...
    def write_binary_file(self, filename, first_address=_DEFAULT_MEM_BOT, 
        ...
        try :        
            ...
            for i in range( first_address, first_address+number_of_bytes ) :
                (c, valid )  = self.read_byte( i )
                filebuf.append(chr(c & 0xFF))
                # Write the file buffer when it's full or if we reached 
                # the end of the data
                if ( len (filebuf) == self._FILE_BUFFER_SIZE or \
                         i ==  (first_address+number_of_bytes-1) ) :
                    f.write( ''.join( filebuf ) )
                    filebuf = []
            f.close()
        except:
            self.error_message.append(
    ...
    def read_intel_hex_file(self, filename, ignore_checksum=False):
        ...
        try :
            ...
            for l in f :
                ...
                if ihex_line_regex.match(l) :
                    ...
                    if record_type == 0 :    ## Standard data record
                        ...
                        for c in data_bytes :
                            max_address = max(max_address,full_address)                            
            ...
            self.memory_image_start = min_address
            self.memory_image_size = (max_address - min_address)+1
            f.close()
        except e:
            self.error_message.append(
    ...
    def read_file( self, filename, format="ihex", address=_DEFAULT_MEM_BOT, 
        ...
        elif format == "binary" or format == "bin" :
    ...
    def write_file( self, filename, format, 
        ...
        if format == "ihex" :
            ...
            self.error_message.append("Unrecognized file format %s - use ihex, hex or bin(ary) only" % format)
        return result
...
def main(argv) :
    ...
    if rslt :
        if output_start_adr < 0 :
            if write_uninitialized_memory:
                output_start_adr = 0
            else:
                output_start_adr = h.memory_image_start
        if output_num_bytes == 0 :
            if write_uninitialized_memory:
                output_num_bytes = _MAX_BUFFER_SIZE - output_start_adr
            else:
                output_num_bytes = h.memory_image_size
